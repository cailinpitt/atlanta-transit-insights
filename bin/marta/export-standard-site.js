...
//
// The frontend (atlanta-transit-alerts) reads this at build time to inject the
...
//
// Source of truth is the local state file written by
// src/marta/shared/standardSite.js as records are published (live + backfill).

require('../../src/shared/env');

const Fs = require('node:fs');
const Path = require('node:path');
const { loadState, documentUri } = require('../../src/marta/shared/standardSite');
...
function writeOutput(out, outputPath) {
  ...
  console.error(
    `marta export-standard-site: wrote ${Object.keys(out.documents).length} documents to ${outputPath}`,
  );
}
