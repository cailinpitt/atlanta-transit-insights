...
// with no `pointAlong`. Generic over bus and rail; the bin passes member rows
// already tagged with disc label + route color group. MARTA reads positions from
// the observe-loop DB (no live polling).
...
async function captureCrossBunchingVideo(
  ...
  const buffer = await encodeFrames(images, { prefix: 'marta-cross-bunch' });
