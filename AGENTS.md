...
- **`@martatraininsights`** — rail (RED/GOLD/BLUE/GREEN) + the Atlanta Streetcar (SC): gaps, bunching, ghosts, speedmaps, adherence, cross-line pileups, system timelapse.
...
| Adherence (early/late) | rail: `src/marta/rail/adherence.js`; bus: `src/marta/bus/adherence.js` + `scripts/marta/build-schedule-stops.js` (schedule.sqlite); rollup `bin/marta/rail/adherence.js`; `docs/ADHERENCE.md` |
