...
// reply, with nothing in between — so an 8-hour outage gives a rider no sense of
// WHY it's still open. This module posts a threaded "still no service — ~Nh in"
// reply once an hour while the incident remains open, and records it into the
// permanent `incident_updates` archive so the event page keeps the timeline.
//
// The text/evidence builders are pure so the live detector bins and the
// backfill (bin/marta/backfill-incident-updates.js) render identical updates;
// `sweepProgressUpdates` is the shared orchestration each bin calls after its
// clear pass, reusing the detector's own Bluesky account.
const incidents = require('./incidents');
const { resolveReplyRef, postText } = require('./bluesky');

// Effective hourly cadence. The bins tick faster (rail pulse every 2 min); the
// 55-min gate collapses that to ~one update per clock hour while tolerating a
// little cron jitter so an update isn't skipped to ~2h.
...
// Reply window: past 24h the original thread is too cold for a fresh reply to
// read naturally (matches the detector clear windows).
...
// "~3h" / "~45 min" — coarse on purpose; updates land hourly so the round-hour
// form reads cleanly, and sub-hour only shows up in tests / odd jitter.
...
// falsy value to skip this incident this tick. `row` is a findUnresolvedDisruptions
// row ({ id, ts, line, direction, evidence, postUri }); `evidence` is its parsed
// firing-time evidence. `getAgent()` returns the logged-in account that owns the
...
async function sweepProgressUpdates({
  ...
  const open = incidents.findUnresolvedDisruptions({ kind, source, sinceMs }, now);
  for (const row of open) {
    const lastUpdateTs = incidents.getLatestIncidentUpdateTs(row.id);
