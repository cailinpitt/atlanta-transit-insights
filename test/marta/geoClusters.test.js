...
const { clusterByProximity, clusterStats } = require('../../src/marta/shared/geoClusters');

// ~364 ft per 0.001 deg latitude — handy for spacing fixtures.
const FT_PER_MILLIDEG_LAT = 364;
const dLatForFt = (ft) => ft / FT_PER_MILLIDEG_LAT / 1000;

const at = (id, ft, route = 'RED') => ({ id, route, lat: 33.75 + dLatForFt(ft), lon: -84.39 });
...
test('drops items without finite lat/lon', () => {
  const items = [at('a', 0), { id: 'x', route: 'GOLD', lat: NaN, lon: -84.39 }, at('b', 100)];
  const clusters = clusterByProximity(items, { radiusFt: 660 });
  assert.equal(clusters.length, 1);
  assert.equal(clusters[0].length, 2);
});
...
test('clusterStats reports span, distinct routes, and centroid', () => {
  const items = [at('a', 0, 'RED'), at('b', 600, 'GOLD'), at('c', 300, 'RED')];
  const { spanFt, routes, centroid } = clusterStats(items, { routeKey: (v) => v.route });
  assert.ok(Math.abs(spanFt - 600) < 5, `spanFt ~600, got ${spanFt}`);
  assert.deepEqual([...routes].sort(), ['GOLD', 'RED']);
  assert.ok(Math.abs(centroid.lat - (33.75 + dLatForFt(300))) < 1e-6);
  assert.ok(Math.abs(centroid.lon - -84.39) < 1e-6);
});
