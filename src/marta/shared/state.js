// Cooldown gate for MARTA posters. Ported from cta-insights src/shared/state.js.
//
// Cooldowns persist in SQLite so overlapping cron instances serialize on the
// file lock. `acquireCooldown` is the only safe gate before posting;
// `isOnCooldown` is a cheap pre-filter where a false-negative gets caught by
// the later acquire. Shares the MARTA DB + cooldowns table with incidents.js.
const { getDb } = require('./incidents');
