...
> **MARTA** rail and the Atlanta Streetcar both use the Path-A approach below: the feeds report no speed, so it's reconstructed from how far a vehicle moved along its line between recorded snapshots (`src/marta/rail/speedmap.js`, fed by `scripts/marta/observe-rail.js`). The streetcar is heavy rail's slow cousin — it reuses the same builder + renderer (`src/marta/streetcar/speedmap.js` wraps `buildLineSpeedmaps` with a single "SC"-keyed loop geometry, a tighter speed cap that rejects the once-per-lap loop wraparound, and its own much slower buckets: red < 4 mph, orange < 8, yellow < 12, purple < 16, green ≥ 16). `bin/marta/rail/speedmap.js` treats the streetcar as **just another line** ("SC") in one candidate pool with RED/GOLD/BLUE/GREEN: a plain run rotates to whichever eligible line has gone longest without a speedmap (recorded under the same `kind: 'rail'` so the rotation is unified). Force a specific one with `--line RED|GOLD|BLUE|GREEN|SC`. Other flags: `--dry-run` (write the image + text, don't post) and `--min-coverage <0–1>` (lower the coverage gate to force a sparse map for inspection). All post to the `@martatraininsights` account.
