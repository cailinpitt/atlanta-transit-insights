#!/usr/bin/env node
// Backfill / reconcile standard.site records for the MARTA archive.
//
...
//
// After a real run, publish the manifest so the site picks up the new tags:
//   node bin/marta/export-standard-site.js <out>   (or run push-web-data.sh)

require('../src/shared/env');

const Fs = require('node:fs');
const { loginAlerts } = require('../src/marta/shared/bluesky');
const { ensurePublication, ensureDocument } = require('../src/marta/shared/standardSite');

const DRY_RUN = process.argv.includes('--dry-run');
const SOURCE =
  process.argv.slice(2).find((a) => !a.startsWith('--')) ||
  `${(process.env.DATA_ORIGIN_URL || 'https://data.atlantatransitalerts.app').replace(/\/$/, '')}/alerts.json`;

// An incident has an /event page only when it carries a Bluesky post (the page
// slug is the post rkey). Postless incidents (e.g. instantly-resolved rollups)
// get no page, so they get no document record.
...
function documentFields(incident) {
  ...
  const title =
    official?.headline ||
    detection?.headline ||
    (routes ? `${routes} ${mode} service disruption` : `MARTA ${mode} service disruption`);
...
async function main() {
  ...
  console.log('Next: run bin/marta/export-standard-site.js (or push-web-data.sh) to publish.');
}
