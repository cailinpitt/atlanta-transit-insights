...
// a disruption after the raw positions roll off (rail_observations keep ~7 days).
//
...
//
// Cron-safe one-shot: read alerts.json → for each replayable rail incident in
// the retention window, extract its positions → gzip → rclone to
// r2atlanta:atlanta-transit-alerts-data/tracks/<id>.json. A manifest skips
// incidents already archived after they resolved; active ones re-upload each run
// until they do. --dry-run writes the JSON to tmp/ and uploads nothing.
//
// Scope: heavy rail only (Red/Gold/Blue/Green). Streetcar positions live in a
// separate table with loop geometry and are out of scope here.
...
const { pickReplayableIncident, buildTrack } = require('../../src/shared/eventTracks');

const REPO = Path.join(__dirname, '..', '..');
const DB_PATH = process.env.MARTA_HISTORY_DB_PATH || Path.join(REPO, 'state', 'marta.sqlite');
const MANIFEST_PATH = Path.join(REPO, 'state', 'track-manifest.json');
const WORK_DIR = Path.join(REPO, 'tmp', 'event-tracks');
const REMOTE = process.env.RCLONE_REMOTE || 'r2atlanta:atlanta-transit-alerts-data';
const ALERTS_URL = process.env.DATA_ORIGIN_URL || 'https://data.atlantatransitalerts.app';
...
// rail_observations roll off at ~7 days; stay inside that so we never archive a
...
async function main() {
  ...
  // Positions for one line over the window. train_id is the per-train identity
  // key; direction (N/S/E/W) is the per-vehicle travel direction the track and
  // the frontend's affectedDir coloring both key off.
  const posQuery = db.prepare(
    `SELECT ts, train_id AS vehicle_id, direction AS dir, lat, lon
       FROM rail_observations
      WHERE line = ? AND ts BETWEEN ? AND ? AND lat IS NOT NULL
      ORDER BY ts`,
  );
  ...
  for (const incident of incidents) {
    ...
    const rows = posQuery.all(picked.lineFeed, qstart, qend);
    if (rows.length === 0) {
      skippedEmpty++;
      continue;
    }

    // MARTA carries a clean travel-direction code (N/S/E/W) on the detection,
    // identical to rail_observations.direction — no destination-text matching
    // needed (unlike CTA's noisy trDr codes).
    const affectedDir = picked.direction;
...
if (require.main === module) {
  ...
  // This is an export job, not a detect→post bin, so it skips runBin's Bluesky
  // web-push flush entirely.
