...
const { shardIncidents, atlantaMonthKey, RECENT_WINDOW_MS } = require('../bin/marta/export-web');
...
test('monthly shards bucket by Atlanta month of first_seen and index every id', () => {
  const incidents = [
...
test('month key honors the Atlanta timezone boundary', () => {
  // 2026-06-01T03:00Z is still 2026-05-31 23:00 in Atlanta (EDT, -4).
  assert.equal(atlantaMonthKey(Date.parse('2026-06-01T03:00:00Z')), '2026-05');
  assert.equal(atlantaMonthKey(Date.parse('2026-06-01T05:00:00Z')), '2026-06');
});
